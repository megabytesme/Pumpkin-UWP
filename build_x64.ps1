# ==============================================================================
#  BUILD SCRIPT: Windows UWP (x64) - 10240 SDK
# ==============================================================================
...
$HybridDir = Join-Path $PWD "libs_x64_temp"
$BuildDir = Join-Path $PWD "target_x64"
...
try {
    ...
    $TemplateLib = Join-Path $SdkBase "um\x64\WindowsApp.lib"
    if (-not (Test-Path $TemplateLib)) { $TemplateLib = Join-Path $SdkBase "um\x64\mincore.lib" }
    ...
    $env:LIB = "$HybridDir;$SdkBase\um\x64;$SdkBase\ucrt\x64"

    Write-Host "Building Pumpkin (x64)..." -ForegroundColor Green
    ...
    $DllPath = Join-Path $BuildDir "x86_64-uwp-windows-msvc\release\pumpkin_uwp.dll"
